...
figma.ui.onmessage = async (msg: any) => {
  ...
  if (msg.type === 'sync-mapped-data') {
    ...
    try {
      ...
      // 5. 只處理 text 映射（圖片由 UI 後續逐張傳送）
      const textMappings = mappings.filter((m: FieldMapping) => m.dataType === 'text');
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        if (existingNode) {
          ...
          await updateInstance(existingNode, record, textMappings);
          updatedCount++;
          ...
          // 填充文字數據
          await fillInstanceData(instance, record, textMappings);
...
function collectLayers(node: SceneNode, prefix = ''): { name: string; type: string; path: string }[] {
  ...
  if (node.name.includes('#')) {
    // 可映射為文字的圖層
    if (node.type === 'TEXT') {
      results.push({ name: node.name, type: 'TEXT', path: fullPath });
    }

    // 可映射為圖片的圖層（Rectangle, Ellipse, Frame, Vector, etc.）
    if (['RECTANGLE', 'ELLIPSE', 'FRAME', 'VECTOR', 'STAR', 'POLYGON', 'LINE'].indexOf(node.type) !== -1) {
...
async function updateInstance(
  ...
  await fillInstanceData(indatanceOrFrame, record, textMappings);
}
